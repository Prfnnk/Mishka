...
export const SIR = () => {
  ...
  const getSign = () => {
    ...
    if (
      (dayNumber >= 21 && dayMonth === 3) ||
      (dayNumber <= 20 && dayMonth === 4)
    ) {
      setSign("Дорогой Овнишка");
    } else if (
      (dayNumber >= 21 && dayMonth === 4) ||
      (dayNumber <= 20 && dayMonth === 5)
    ) {
      console.log(dayMonth);
      setSign("Дорогой Телец-молодец");
    } else if (
      (dayNumber >= 21 && dayMonth === 5) ||
      (dayNumber <= 20 && dayMonth === 6)
    ) {
      setSign("Дорогой Близняшка");
    } else if (
      (dayNumber >= 21 && dayMonth === 6) ||
      (dayNumber <= 22 && dayMonth === 7)
    ) {
      setSign("Дорогой Рак");
    } else if (
      (dayNumber >= 23 && dayMonth === 7) ||
      (dayNumber <= 22 && dayMonth === 8)
    ) {
      setSign("Дорогой Львёнок");
    } else if (
      (dayNumber >= 23 && dayMonth === 8) ||
      (dayNumber <= 22 && dayMonth === 9)
    ) {
      setSign("Дорогая Дева");
    } else if (
      (dayNumber >= 23 && dayMonth === 9) ||
      (dayNumber <= 22 && dayMonth === 10)
    ) {
      setSign("Дорогие Весы");
    } else if (
      (dayNumber >= 23 && dayMonth === 10) ||
      (dayNumber <= 22 && dayMonth === 11)
    ) {
      setSign("Дорогой Скорпиончик");
    } else if (
      (dayNumber >= 23 && dayMonth === 11) ||
      (dayNumber <= 21 && dayMonth === 12)
    ) {
      setSign("Дорогой Срелец-удалец");
    } else if (
      (dayNumber >= 22 && dayMonth === 12) ||
      (dayNumber <= 19 && dayMonth === 1)
    ) {
      setSign("Дорогой Козерожка");
    } else if (
      (dayNumber >= 20 && dayMonth === 1) ||
      (dayNumber <= 19 && dayMonth === 2)
    ) {
      setSign("Дорогой Водолей");
    } else if (
      (dayNumber >= 20 && dayMonth === 2) ||
      (dayNumber <= 20 && dayMonth === 3)
    ) {
  ...
  return (
    <Wrapper>
      ...
      <Discription>
        В тяжелые вирусные времена как никогда нужна уверенность в своем
        будущем..Как быть? Введи дату своего рождения и все станет ясно как
        день!
      </Discription>
      <DateBlock>
        <Input
        ...
        />
        <Button type="button" onClick={() => getSign()}>
          Узнать!
        </Button>
      </DateBlock>
      {sign && showPage ? (
        <HiddenBlock>
          ...
          <ResultBlock>
            {firstSentence[randomIndex()]} {secondSentence[randomIndex()]}{" "}
            {thirdSentence[randomIndex()]}
          </ResultBlock>
        </HiddenBlock>
      ) : null}
    </Wrapper>
